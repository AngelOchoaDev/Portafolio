...
const ContactButton = ( { key , direccion , tipo, texto , logo } ) => {
  ...
  return (
      <a
        href={`${ (tipo === "correo") ? "mailto:" : ( tipo === "telefono" ) ? "https://api.whatsapp.com/send?phone=" : "https://"}${ tipo === "correo" ? encodeURIComponent(direccion) : direccion }${ (tipo === "correo") ? ("?subject="+encodeURIComponent("Hola, te contacto desde tu página web.")+"&body="+encodeURIComponent("Buen día!\n\nMe gustaría hablar contigo sobre...")) : ( tipo === "telefono" ) ? "&text="+encodeURIComponent("Hola, buenas tardes. Te contacto desde tu pagina web. Me gustaría hablar contigo sobre...") : "" }`}
        target="_blank"
        rel="noreferrer"
